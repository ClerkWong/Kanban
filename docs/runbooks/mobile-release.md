...
Card schema v8 之前的客戶端（mobile build 7 及更早）送出的 board 不含
`assignmentWindows`。以 **Project owner** 身分用舊客戶端編輯已排期的看板時，Worker 會
接受（owner 有權變更指派），該看板全部的投入期間會被**靜默抹除**；member 則會被 403
擋下，屬 fail-safe。

因此 Web Beta 發布 v8 之後：

1. 管理者裝置必須先重新載入頁面，再開始排期。
2. 舊的 mobile build 在新版安裝前只讀不改，尤其不要以 owner 身分編輯卡片。
3. 行動版更新應盡快發出——這不只是功能更新，也是資料完整性修正。
